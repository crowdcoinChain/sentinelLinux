...
class GovernanceClass(object):
    ...
    def vote(self, crowdcoind, signal, outcome):
        return self.go.vote(crowdcoind, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, crowdcoind):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(crowdcoind, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(crowdcoind, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to crowdcoind "gobject list" output
    ...
    def crowdcoind_serialise(self):
        import crowdcoinlib
        return crowdcoinlib.SHIM_serialise_for_crowdcoind(self.serialise())
