...
class CrowdcoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some crowdcoind methods
    ...
    @classmethod
    def from_crowdcoin_conf(self, crowdcoin_dot_conf):
        from crowdcoin_config import CrowdcoinConfig
        config_text = CrowdcoinConfig.slurp_config_file(crowdcoin_dot_conf)
        creds = CrowdcoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = crowdcoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = crowdcoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = crowdcoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_crowdcoind_proto_version_with_sentinel_ping)
